...
class Polytope:
  ...
  def determine_H_rep(self):
    ...
    if not self.is_full_dimensional or self.n < 2:  # TODO: fix this
      ...
      i_ineq_rows = list(set(range(H_cdd.row_size)) - H_cdd.lin_set)
  ...
  def contains(self, Q):
    ...
    if isinstance(Q, Polytope):  # P and Q are both polytopes
      ...
      # holds for all x.
      Ax_m_b = np.matmul(self.A, x) - self.b
...
def P_plus_p(P, point, subtract_p=False):
  ...
  if P.in_H_rep:
    P_shifted._set_Ab(P.A, P.b + np.matmul(P.A, p))  # TODO: redesign this
  return P_shifted
...
def linear_map(M, P):
  ...
  return Polytope(np.matmul(P.V, M.T))
...
def redundant_inequalities(A, b):
  ...
  #   model = gp.Model("mip1")
  #   model.setParam('OutputFlag', False)
  #   x = np.array([model.addVar(name="x"+str(i), vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY) for i in range(A.shape[1])])
  #   cost = A[ineq].dot(x)

  #   for t in range(A.shape[0]):
  #     if ~removed[t]:
  #       model.addConstr(A[t].dot(x) <= b_plus_1[t, ineq])
  ...
  #   value = 0
  #   if model.status == GRB.OPTIMAL:
  #   	value = sum(A[ineq][i] * x[i].X for i in range(A.shape[1]))
  #   else:
  #   	print('here')

  #   if value <= b[ineq] or np.isclose(value, b[ineq]):
  #     removed[ineq] = True
